...
import { Package, ShoppingCart, DollarSign, Users, TrendingUp, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: Users,
      trend: analytics?.data?.conversionRate > 0 ? "up" : "down",
    },
  ];
      ...
      <div className="grid gap-4 md:grid-cols-2">
          ...
          <CardContent>
            {analytics?.data?.recentOrders?.length > 0 ? (
              <div className="space-y-4">
                {analytics.data.recentOrders.slice(0, 5).map((order: any) => (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              Top Drops
              <Link href="/drops" className="text-sm font-normal text-primary hover:underline">
                View all
              </Link>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {analytics?.data?.topDrops?.length > 0 ? (
              <div className="space-y-4">
                {analytics.data.topDrops.slice(0, 5).map((drop: any) => (
                  <div key={drop.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <Badge variant={drop.status === "LIVE" ? "success" : "secondary"}>
                        {drop.status}
                      </Badge>
                    </div>
                  </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{((analytics?.data?.conversionRate || 0) * 100).toFixed(1)}%</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Launch a new product</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Offer special deals</p>
                </div>
              </div>
            </CardContent>
          </Card>
